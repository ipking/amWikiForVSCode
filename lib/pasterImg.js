...
class pasterImg {
    ...
    /**
     * @desc 查找指定文件夹
     * @param {String} rootPath 当前工作文件夹所在路径
     * @param {String} resFolder 图片文件夹名
     * @param {String} selectText  被光标选中的文本
     * @return {String} 返回获得的图片文件路径信息 
     */
    getImgPathForSpecFolder(rootPath, resFolder, selectText) {
        var date = new Date();
        var assertPath = rootPath;
        var subList = [resFolder, date.Format("yyyy-MM-dd")];
        var nCount = subList.length;
        for (var i = 0; i < nCount; i++) {
            assertPath = path.join(assertPath, subList[i]);
        ...
        return path.join(assertPath, imageFileName);
    }
